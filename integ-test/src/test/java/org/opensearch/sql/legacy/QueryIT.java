...
public class QueryIT extends SQLIntegTestCase {
   ...
   *
   * <p>Also ignoring the following tests as they are failing, will require investigation: -
   * idsQuerySubQueryIds - escapedCharactersCheck - fieldCollapsingTest - idsQueryOneId -
   * idsQueryMultipleId - multipleIndicesOneNotExistWithoutHint
   *
   * <p>The following tests are being ignored because subquery is still running in OpenSearch
   * transport thread: - twoSubQueriesTest() - inTermsSubQueryTest()
   */
        ...
        executeQuery(
            String.format(
                ...
                "SELECT insert_time FROM %s where script('doc[\\'insert_time\''].date.hourOfDay==16') "
                    + "and insert_time <'2014-08-21T00:00:00.000Z'",
                TEST_INDEX_ONLINE));
  ...
  @Ignore(
      "Goes in different route, does not call PrettyFormatRestExecutor.execute methods."
