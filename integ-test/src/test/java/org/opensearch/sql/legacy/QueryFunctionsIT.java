...
public class QueryFunctionsIT extends SQLIntegTestCase {
  ...
  /**
   * TODO Looks like Math/Date Functions test all use the same query() and execute() functions TODO
   * execute/featureValueOf/hits functions are the same as used in NestedFieldQueryIT, should
   * refactor into util
   */
  @Override
  ...
  /**
   * Create Matchers for each field and its value Only one of the Matchers need to match (per hit)
   *
   * <p>Ex. If a query with wildcard field is made: multi_match(query="Ayers", fields="*name")
   *
   * <p>Then the value "Ayers" can be found in either the firstname or lastname field. Only one of
   * these fields need to satisfy the query value to be evaluated as correct expected output.
   *
