...
public class MathFunctionsIT extends SQLIntegTestCase {
  ...
  public void lowerCaseFunctionCall() throws IOException {
    SearchHit[] hits = query("SELECT abs(age - 100) AS abs");
  ...
  public void eulersNumber() throws IOException {
    SearchHit[] hits = query("SELECT E() AS e");
  ...
  public void pi() throws IOException {
    SearchHit[] hits = query("SELECT PI() AS pi");
  ...
  public void expm1Function() throws IOException {
    SearchHit[] hits = query("SELECT EXPM1(2) AS expm1");
  ...
  public void degreesFunction() throws IOException {
    SearchHit[] hits = query("SELECT age, DEGREES(age) AS degrees");
    for (SearchHit hit : hits) {
  ...
  public void radiansFunction() throws IOException {
    SearchHit[] hits = query("SELECT age, RADIANS(age) as radians");
    for (SearchHit hit : hits) {
      int age = (int) getFieldFromSource(hit, "age");
  ...
  public void sin() throws IOException {
    SearchHit[] hits = query("SELECT SIN(PI()) as sin");
  ...
  public void asin() throws IOException {
    SearchHit[] hits = query("SELECT ASIN(PI()) as asin");
  ...
  public void sinh() throws IOException {
    SearchHit[] hits = query("SELECT SINH(PI()) as sinh");
  ...
  public void atan2() throws IOException {
    SearchHit[] hits =
        query(
            "SELECT ATAN2(age, age) AS atan2", "WHERE (age IS NOT NULL) AND (ATAN2(age, age) > 0)");
  ...
  public void cot() throws IOException {
    SearchHit[] hits = query("SELECT COT(PI()) AS cot");
  ...
  public void sign() throws IOException {
    SearchHit[] hits = query("SELECT SIGN(E()) AS sign");
    ...
    assertThat(
        executeQuery(
            "SELECT LOG(age) FROM "
                ...
                + " WHERE age IS NOT NULL GROUP BY LOG(age) ORDER BY LOG(age)",
            "jdbc"),
        containsString("\"type\": \"double\""));
    assertThat(
        executeQuery(
            "SELECT LOG(2, age) FROM "
                + TestsConstants.TEST_INDEX_ACCOUNT
                + " WHERE age IS NOT NULL GROUP BY LOG(2, age) ORDER BY LOG(2, age)",
            "jdbc"),
        containsString("\"type\": \"double\""));
  }
  ...
  public void lnInAggregationShouldPass() {
    assertThat(
        executeQuery(
            "SELECT LN(age) FROM "
                + TestsConstants.TEST_INDEX_ACCOUNT
                + " WHERE age IS NOT NULL GROUP BY LN(age) ORDER BY LN(age)",
            "jdbc"),
        containsString("\"type\": \"double\""));
  }
