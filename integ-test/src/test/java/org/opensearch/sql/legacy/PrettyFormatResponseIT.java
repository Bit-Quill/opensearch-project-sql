...
/**
 * PrettyFormatResponseIT will likely be excluding some of the tests written in
 * PrettyFormatResponseTest since those tests were asserting on class objects directly. These
 * updated tests will only be making assertions based on the REST response.
 *
 * <p>Any integ tests from PrettyFormatResponseTest that were excluded can perhaps later be changed
 * and moved over to be unit tests.
 *
 * <p>Tests from original integ tests excluded: - noIndexType() - withIndexType()
 */
public class PrettyFormatResponseIT extends SQLIntegTestCase {

  private static final Set<String> allAccountFields =
      Sets.newHashSet(
          "account_number",
          "balance",
          "firstname",
  ...
  private static final Set<String> messageFields =
      Sets.newHashSet("message.dayOfWeek", "message.info", "message.author");

  private static final Set<String> messageFieldsWithNestedFunction =
      Sets.newHashSet(
  ...
  public void selectAll() throws IOException {
    JSONObject response =
        executeQuery(
            String.format(Locale.ROOT, "SELECT * FROM %s", TestsConstants.TEST_INDEX_ACCOUNT));
  ...
  public void selectWrongField() throws IOException {
    JSONObject response =
        executeQuery(
            String.format(
                Locale.ROOT, "SELECT wrongField FROM %s", TestsConstants.TEST_INDEX_ACCOUNT));

    assertThat(getSchema(response).length(), equalTo(0));

    // DataRows object will still get populated with SearchHits but since wrongField is not
  ...
  /**
   * Execute the query against index with nested fields and assert result contains columns and data
   * as expected.
   */
  @SafeVarargs
  private final void assertNestedFieldQueryResultContainsColumnsAndData(
      String query, Set<String>... expectedFieldNames) throws IOException {
    JSONObject response =
        executeQuery(String.format(Locale.ROOT, query, TestsConstants.TEST_INDEX_NESTED_TYPE));
    Set<String> allExpectedFieldNames =
        Stream.of(expectedFieldNames).flatMap(Set::stream).collect(toSet());
  ...
  public void selectNestedFields() throws IOException {
    ...
    // The nested test index being used contains 5 entries but one of them has an array of 2 message
    // objects, so
  ...
  public void testSizeAndTotal() throws IOException {
    ...
    // The value to compare to here was obtained by running the query in the plugin and looking at
    // the SearchHits
  ...
  public void testSizeWithGroupBy() throws IOException {
    JSONObject response =
        executeQuery(
            String.format(
                Locale.ROOT,
                "SELECT * FROM %s GROUP BY age LIMIT 5",
                TestsConstants.TEST_INDEX_ACCOUNT));
  ...
  /**
   * This case doesn't seem to be supported by the plugin at the moment. Looks like the painless
   * script of the inner function is put inside the aggregation function but this syntax may not be
   * correct since it returns 0 which is the default value (since 0 is returned in cases like
   * COUNT(wrongField) as well).
   */
  //    @Test
  //    public void nestedAggregationFunctionInSelect() {
  //        String query = String.format(Locale.ROOT, "SELECT SUM(SQRT(age)) FROM age GROUP BY age",
  // TEST_INDEX_ACCOUNT);
  //    }
  @Test
  public void fieldsWithAlias() throws IOException {
    JSONObject response =
        executeQuery(
            String.format(
                Locale.ROOT,
                "SELECT firstname AS first, age AS a FROM %s",
                TestsConstants.TEST_INDEX_ACCOUNT));
  ...
  public void indexWithMissingFields() throws IOException {
    ...
    assertThat(
        dataRowEntry.get(1), equalTo(JSONObject.NULL)); // TODO See if this null check is failing
  }
  ...
  public void joinQuery() throws IOException {
    JSONObject response =
        executeQuery(
            String.format(
                Locale.ROOT,
                "SELECT b1.balance, b1.age, b2.firstname "
  ...
  public void joinQueryWithObjectFieldInSelect() throws IOException {
    JSONObject response =
        executeQuery(
            String.format(
                Locale.ROOT,
                "SELECT c.name.firstname, d.name.lastname "
