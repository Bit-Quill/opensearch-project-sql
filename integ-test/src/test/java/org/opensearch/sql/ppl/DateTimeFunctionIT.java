...
public class DateTimeFunctionIT extends PPLIntegTestCase {
  ...
  public void testAddTime() throws IOException {
    var result = executeQuery(String.format("source=%s | eval"
        ...
        + " `'15:42:13' + '09:07:00'` = ADDTIME(TIMESTAMP('1999-12-31 15:42:13'), TIMESTAMP('1961-04-12 09:07:00'))"
  ...
  public void testSubTime() throws IOException {
    var result = executeQuery(String.format("source=%s | eval"
        ...
        + " `'15:42:13' - '09:07:00'` = SUBTIME(TIMESTAMP('1999-12-31 15:42:13'), TIMESTAMP('1961-04-12 09:07:00'))"
        + " | fields `'2008-12-12' - 0`, `'23:59:59' - 0`, `'2004-01-01' - '23:59:59'`, `'10:20:30' - '00:05:42'`, `'15:42:13' - '09:07:00'`", TEST_INDEX_DATE));
    verifySchema(result,
        schema("'2008-12-12' - 0", null, "timestamp"),
        schema("'23:59:59' - 0", null, "time"),
  ...
  public void testDateDiff() throws IOException {
    var result = executeQuery(String.format("source=%s | eval"
        ...
        + " `'2004-01-01' - '2002-02-01'` = DATEDIFF(TIMESTAMP('2004-01-01 00:00:00'), TIMESTAMP('2002-02-01 14:25:30')),"
  ...
  public void testToSeconds() throws IOException{
    var result = executeQuery(String.format("source=%s | eval f1 = to_seconds(date('2008-10-07')) | " +
            ...
            "eval f3 = to_seconds(TIMESTAMP('2020-09-16 07:40:00')) | fields f1, f2, f3", TEST_INDEX_DATE));
    verifySchema(result,
            schema("f1", null, "long"),
            schema("f2", null, "long"),
            schema("f3", null, "long"));
