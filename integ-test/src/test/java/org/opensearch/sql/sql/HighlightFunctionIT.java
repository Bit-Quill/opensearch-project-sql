...
public class HighlightFunctionIT extends SQLIntegTestCase {
  ...
  public void highlight_multiple_optional_arguments_test() {
    ...
    verifyDataRows(
        ...
        rows(
            ...
            new JSONArray(
                List.of(
                    "<p>I know what makes an <mark style=\"background-color: green;\">IPA</mark>"
                        + " an <mark style="
  ...
  public void multiple_highlight_test() {
    String query =
        "SELECT highlight(Title), highlight(Tags) FROM %s WHERE MULTI_MATCH([Title, Tags], 'hops') LIMIT 1";
    JSONObject response = executeJdbcRequest(String.format(query, TestsConstants.TEST_INDEX_BEER));
    verifySchema(
        response,
        schema("highlight(Title)", null, "nested"),
  ...
  public void highlight_no_limit_test() {
    ...
    verifyDataRows(
        ...
        rows(
            new JSONArray(
                List.of(
                    "<p>Do <em>hops</em> have (or had in the past) any use outside of brewing beer?",
