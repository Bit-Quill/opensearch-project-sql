...
public class DateFormatTest {
  ...
  public void simpleFormatCondition() {
    List<QueryBuilder> q =
        query(SELECT_CNT_FROM_DATE + "WHERE date_format(creationDate, 'YYYY') < '2018'");

    assertThat(q, hasQueryWithValue("fieldName", equalTo("creationDate")));
    assertThat(
        q,
        hasQueryWithValueGetter(
            MatcherUtils.featureValueOf(
                "has format", equalTo("YYYY"), f -> ((RangeQueryBuilder) f).format())));
  }
  ...
  public void equalCondition() {
    List<QueryBuilder> q =
        query(
            SELECT_CNT_FROM_DATE + "WHERE date_format(creationDate, 'YYYY-MM-dd') = '2018-04-02'");

    assertThat(
        q,
        hasQueryWithValueGetter(
            MatcherUtils.featureValueOf(
                "has format", equalTo("YYYY-MM-dd"), f -> ((RangeQueryBuilder) f).format())));

    // Equality query for date_format is created with a rangeQuery where the 'from' and 'to' values
    // are equal to the value we are equating to
    assertThat(
        q,
        hasQueryWithValue(
            "from",
            equalTo(
                BytesRefs.toBytesRef(
                    "2018-04-02")))); // converting string to bytes ref as RangeQueryBuilder stores
    // it this way
  ...
  public void orderByTest() {
    String query =
        "SELECT agent, ip, date_format(utc_time, 'dd-MM-YYYY') date "
  ...
  public void groupByWithDescOrder() throws SqlParseException {
    String query =
        "SELECT date_format(utc_time, 'dd-MM-YYYY'), count(*) "
            + "FROM opensearch_dashboards_sample_data_logs "
  ...
  public void groupByWithAndAlias() throws SqlParseException {
    String query =
        "SELECT date_format(utc_time, 'dd-MM-YYYY') x, count(*) "
            + "FROM opensearch_dashboards_sample_data_logs "
            + "GROUP BY x "
            + "ORDER BY x";
  ...
  public void timeZoneCondition() {
    List<QueryBuilder> q =
        query(
            SELECT_CNT_FROM_DATE
                + "WHERE date_format(creationDate, 'YYYY-MM-dd', 'America/Phoenix') > '2018-04-02'");

    // Used hasProperty here as getter followed convention for obtaining ID and Feature Matcher was
    // having issues with generic type to obtain value
  ...
  private <T, U> Matcher<Iterable<? super T>> hasQueryWithValue(
      String name, Matcher<? super U> matcher) {
    return hasItem(
        hasFieldWithValue(
            "mustClauses",
            "has mustClauses",
  ...
  private <T, U> Matcher<Iterable<? super T>> hasNotQueryWithValue(
      String name, Matcher<? super U> matcher) {
    return hasItem(
        hasFieldWithValue(
            "mustClauses",
  ...
  private <T, U> Matcher<Iterable<? super T>> hasQueryWithValueGetter(Matcher<? super U> matcher) {
    return hasItem(hasFieldWithValue("mustClauses", "has mustClauses", hasItem(matcher)));
  }
}
