...
public class SqlRequestFactoryTest {
  ...
  public void testGenerateSqlRequest_preparedStatementFromPayload() {
    ...
    Assert.assertEquals(
        "select * from my_table where int_param = ? and double_param = ? and string_param = ? and date_param = ? and null_param = ?",
        preparedStatementRequest.getPreparedStatement());
    Assert.assertEquals(
        "select * from my_table where int_param = 1 and double_param = 2.0 and string_param = 'string_value' and date_param = '2000-01-01' and null_param = null",
