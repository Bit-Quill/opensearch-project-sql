...
public class CursorResultExecutor implements CursorRestExecutor {
  ...
  private String handleDefaultCursorRequest(Client client, DefaultCursor cursor) {
    ...
    if (rowsLeft < fetch && rowsLeft < searchHitArray.length) {
       ...
       * from SearchHit[] Avoid truncating in following two scenarios 1. number of rows to be sent
       * equals fetchSize 2. size of SearchHit[] is already less that rows that needs to be sent
       *
       * <p>Else truncate to desired number of rows
       */
