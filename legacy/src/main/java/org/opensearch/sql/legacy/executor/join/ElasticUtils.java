...
public class ElasticUtils {
  ...
  // use our deserializer instead of results toXcontent because the source field is different from
  // sourceAsMap.
  public static String hitsAsStringResult(SearchHits results, MetaSearchResult metaResults)
      throws IOException {
    if (results == null) {
      return null;
    }
    ...
    builder.field(
        ...
        ImmutableMap.of(
            "total",
            metaResults.getTotalNumOfShards(),
            "successful",
            metaResults.getSuccessfulShards(),
            "failed",
            metaResults.getFailedShards()));
  ...
  public static XContentBuilder hitsAsStringResultZeroCopy(
      List<SearchHit> results, MetaSearchResult metaResults, ElasticJoinExecutor executor)
      throws IOException {
    BytesStreamOutput outputStream = new BytesStreamOutput();

    XContentBuilder builder =
        XContentFactory.contentBuilder(XContentType.JSON, outputStream).prettyPrint();
    builder.startObject();
    builder.field("took", metaResults.getTookImMilli());
    builder.field("timed_out", metaResults.isTimedOut());
    builder.field(
  ...
  private static void toXContent(
      XContentBuilder builder, Params params, List<SearchHit> hits, ElasticJoinExecutor executor)
      throws IOException {
    builder.startObject(SearchHits.Fields.HITS);
    builder.field(
        SearchHits.Fields.TOTAL,
        ImmutableMap.of("value", hits.size(), "relation", Relation.EQUAL_TO));
  ...
  private static void toXContent(XContentBuilder builder, Params params, SearchHit hit)
      throws IOException {
    builder.startObject();
    if (hit.getId() != null) {
