...
public class CanPaginateVisitor extends AbstractNodeVisitor<Boolean, Object> {
    ...
    public Boolean visitSort(Sort node, Object context) {
        return node.getSortList().stream().allMatch(f -> f.getField() instanceof QualifiedName && visitField(f, context))
    ...
    public Boolean visitField(Field node, Object context) {
        return canPaginate(node, context) && node.getFieldArgs().stream().allMatch(n -> n.accept(this, context));
    }
    ...
    public Boolean visitIn(In node, Object context) {
        return canPaginate(node, context) && node.getValueList().stream().allMatch(n -> n.accept(this, context));
    }
