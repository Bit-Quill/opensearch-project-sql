...
public class MicroBatchStreamingExecution {
    ...
    public void execute() {
        ...
        Long latestCommittedBatchId = committedLog.getLatest().map(Pair::getKey).orElse(INITIAL_LATEST_BATCH_ID);
        ...
        if (latestBatchId.equals(latestCommittedBatchId)) {
            ...
            Preconditions.checkArgument(
                ...
                "[BUG] Expected latestBatchId - latestCommittedBatchId = 0 or 1, " + "but latestBatchId=%d, latestCommittedBatchId=%d",
                latestBatchId,
        ...
        if (hasNewData(availableOffsets, committedOffset)) {
            ...
            queryService.executePlan(batchPlan, new PlanContext(batch.getSplit()), new ResponseListener<>() {
                @Override
