 ...
 */
public class Explain extends PhysicalPlanNodeVisitor<ExplainResponseNode, Object> implements Function<PhysicalPlan, ExplainResponse> {
    ...
    public ExplainResponseNode visitProject(ProjectOperator node, Object context) {
        return explain(
            node,
            context,
            explainNode -> explainNode.setDescription(ImmutableMap.of("fields", node.getProjectList().toString()))
        );
    }
    ...
    public ExplainResponseNode visitFilter(FilterOperator node, Object context) {
        return explain(
            node,
            context,
            explainNode -> explainNode.setDescription(ImmutableMap.of("conditions", node.getConditions().toString()))
        );
    }
    ...
    public ExplainResponseNode visitSort(SortOperator node, Object context) {
        return explain(
            node,
            context,
            explainNode -> explainNode.setDescription(ImmutableMap.of("sortList", describeSortList(node.getSortList())))
        );
    }

    @Override
    public ExplainResponseNode visitTableScan(TableScanOperator node, Object context) {
        return explain(node, context, explainNode -> explainNode.setDescription(ImmutableMap.of("request", node.toString())));
    }
    ...
    public ExplainResponseNode visitAggregation(AggregationOperator node, Object context) {
        return explain(
            ...
            explainNode -> explainNode.setDescription(
                ImmutableMap.of("aggregators", node.getAggregatorList().toString(), "groupBy", node.getGroupByExprList().toString())
            )
        );
    }
    ...
    public ExplainResponseNode visitWindow(WindowOperator node, Object context) {
        return explain(
            ...
            explainNode -> explainNode.setDescription(
                ImmutableMap.of(
                    "function",
                    node.getWindowFunction().toString(),
                    "definition",
                    ImmutableMap.of(
                        "partitionBy",
    ...
    public ExplainResponseNode visitRename(RenameOperator node, Object context) {
        Map<String, String> renameMappingDescription = node.getMapping()
            .entrySet()
            .stream()
            .collect(Collectors.toMap(e -> e.getKey().toString(), e -> e.getValue().toString()));

        return explain(node, context, explainNode -> explainNode.setDescription(ImmutableMap.of("mapping", renameMappingDescription)));
    }
    ...
    public ExplainResponseNode visitRemove(RemoveOperator node, Object context) {
        return explain(
            node,
            context,
            explainNode -> explainNode.setDescription(ImmutableMap.of("removeList", node.getRemoveList().toString()))
        );
    }
    ...
    public ExplainResponseNode visitEval(EvalOperator node, Object context) {
        return explain(
            node,
            context,
            explainNode -> explainNode.setDescription(ImmutableMap.of("expressions", convertPairListToMap(node.getExpressionList())))
        );
    }
        ...
        return explain(
            ...
            explainNode -> explainNode.setDescription(
                ImmutableMap.of(
                    ...
                    node.getNoOfResults(),
                    "fields",
                    node.getFieldExprList().toString(),
                    "groupBy",
                    node.getGroupByExprList().toString()
                )
            )
        );
    }
    ...
    public ExplainResponseNode visitValues(ValuesOperator node, Object context) {
        return explain(node, context, explainNode -> explainNode.setDescription(ImmutableMap.of("values", node.getValues())));
    }
    ...
    public ExplainResponseNode visitLimit(LimitOperator node, Object context) {
        return explain(
            node,
            context,
            explanNode -> explanNode.setDescription(ImmutableMap.of("limit", node.getLimit(), "offset", node.getOffset()))
        );
    }

    @Override
    public ExplainResponseNode visitNested(NestedOperator node, Object context) {
        return explain(node, context, explanNode -> explanNode.setDescription(ImmutableMap.of("nested", node.getFields())));
    }

    protected ExplainResponseNode explain(PhysicalPlan node, Object context, Consumer<ExplainResponseNode> doExplain) {
        ...
        return sortList.stream()
            .collect(
                Collectors.toMap(
                    ...
                    p -> ImmutableMap.of(
                        "sortOrder",
                        p.getLeft().getSortOrder().toString(),
                        "nullOrder",
                        p.getLeft().getNullOrder().toString()
                    )
                )
            );
    }
