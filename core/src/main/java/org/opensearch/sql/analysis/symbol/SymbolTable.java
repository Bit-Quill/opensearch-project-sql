...
public class SymbolTable {
     ...
     */
    private Map<Namespace, NavigableMap<String, ExprType>> tableByNamespace = new EnumMap<>(Namespace.class);
     ...
     * symbol.
     */
    private Map<Namespace, LinkedHashMap<String, ExprType>> orderedTable = new EnumMap<>(Namespace.class);
    ...
    public void store(Symbol symbol, ExprType type) {
        tableByNamespace.computeIfAbsent(symbol.getNamespace(), ns -> new TreeMap<>()).put(symbol.getName(), type);

        orderedTable.computeIfAbsent(symbol.getNamespace(), ns -> new LinkedHashMap<>()).put(symbol.getName(), type);
    }
    ...
    public void remove(Symbol symbol) {
        tableByNamespace.computeIfPresent(symbol.getNamespace(), (k, v) -> {
            v.remove(symbol.getName());
            return v;
        });
        orderedTable.computeIfPresent(symbol.getNamespace(), (k, v) -> {
            v.remove(symbol.getName());
            return v;
        });
    }
    ...
    public Map<String, ExprType> lookupAllFields(Namespace namespace) {
        final LinkedHashMap<String, ExprType> allSymbols = orderedTable.getOrDefault(namespace, new LinkedHashMap<>());
