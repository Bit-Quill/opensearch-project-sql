...
public class SelectExpressionAnalyzer extends AbstractNodeVisitor<List<NamedExpression>, AnalysisContext> {
    ...
    public List<NamedExpression> visitAlias(Alias node, AnalysisContext context) {
        ...
        return Collections.singletonList(DSL.named(unqualifiedNameIfFieldOnly(node, context), expr, node.getAlias()));
    }
    ...
    private Expression referenceIfSymbolDefined(Alias expr, AnalysisContext context) {
        ...
        return optimizer.optimize(DSL.named(expr.getName(), delegatedExpr.accept(expressionAnalyzer, context), expr.getAlias()), context);
    }
    ...
    public List<NamedExpression> visitAllFields(AllFields node, AnalysisContext context) {
        ...
        return lookupAllFields.entrySet()
            .stream()
            .map(entry -> DSL.named(entry.getKey(), new ReferenceExpression(entry.getKey(), entry.getValue())))
    ...
    public List<NamedExpression> visitNestedAllTupleFields(NestedAllTupleFields node, AnalysisContext context) {
        ...
        return lookupAllTupleFields.entrySet().stream().filter(field -> p.matcher(field.getKey()).find()).map(entry -> {
            Expression nestedFunc = new Function("nested", List.of(new QualifiedName(List.of(entry.getKey().split("\\."))))).accept(
                expressionAnalyzer,
                context
            );
