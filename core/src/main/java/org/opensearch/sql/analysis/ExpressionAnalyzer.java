...
public class ExpressionAnalyzer extends AbstractNodeVisitor<Expression, AnalysisContext> {
    ...
    public Expression visitCast(Cast node, AnalysisContext context) {
        ...
        return (Expression) repository.compile(
    ...
    public Expression visitFunction(Function node, AnalysisContext context) {
        ...
        List<Expression> arguments = node.getFuncArgs().stream().map(unresolvedExpression -> {
        ...
        return (Expression) repository.compile(context.getFunctionProperties(), functionName, arguments);
    }
    ...
    public Expression visitScoreFunction(ScoreFunction node, AnalysisContext context) {
        ...
        if (!boostArg.getType().equals(DataType.DOUBLE)) {
            throw new SemanticCheckException(
                String.format("Expected boost type '%s' but got '%s'", DataType.DOUBLE.name(), boostArg.getType().name())
            );
        }
    ...
    public Expression visitCompare(Compare node, AnalysisContext context) {
        ...
        return (Expression) repository.compile(context.getFunctionProperties(), functionName, Arrays.asList(left, right));
    }

    @Override
    public Expression visitBetween(Between node, AnalysisContext context) {
        return and(compare(">=", node.getValue(), node.getLowerBound()), compare("<=", node.getValue(), node.getUpperBound())).accept(
            this,
            context
        );
    }
    ...
    public Expression visitCase(Case node, AnalysisContext context) {
        ...
        for (When when : node.getWhenClauses()) {
            if (node.getCaseValue() == null) {
                ...
                whens.add(
                    (WhenClause) analyze(
                        new When(new Function("=", Arrays.asList(node.getCaseValue(), when.getCondition())), when.getResult()),
                        context
                    )
                );
            }
        ...
        Expression defaultResult = (node.getElseClause() == null) ? null : analyze(node.getElseClause(), context);
    ...
    public Expression visitWhen(When node, AnalysisContext context) {
        return new WhenClause(analyze(node.getCondition(), context), analyze(node.getResult(), context));
    }
    ...
    public Expression visitQualifiedName(QualifiedName node, AnalysisContext context) {
        ...
        for (String part : node.getParts()) {
            for (TypeEnvironment typeEnv = context.peek(); typeEnv != null; typeEnv = typeEnv.getParent()) {
                Optional<ExprType> exprType = typeEnv.getReservedSymbolTable().lookup(new Symbol(Namespace.FIELD_NAME, part));
                if (exprType.isPresent()) {
                    return visitMetadata(qualifierAnalyzer.unqualified(node), (ExprCoreType) exprType.get(), context);
                }
            }
        }
