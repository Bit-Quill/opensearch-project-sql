...
public enum ExprCoreType implements ExprType {
     ...
     */
    private static final Map<ExprCoreType, String> LEGACY_TYPE_NAME_MAPPING = new ImmutableMap.Builder<ExprCoreType, String>().put(
        STRUCT,
        "OBJECT"
    ).put(ARRAY, "NESTED").put(STRING, "KEYWORD").build();

    private static final Set<ExprType> NUMBER_TYPES = new ImmutableSet.Builder<ExprType>().add(BYTE)
        .add(SHORT)
        .add(INTEGER)
        .add(LONG)
        .add(FLOAT)
