    ...
    private static FunctionResolver nested() {
        return new FunctionResolver() {
            ...
            public Pair<FunctionSignature, FunctionBuilder> resolve(FunctionSignature unresolvedSignature) {
                return Pair.of(
                    unresolvedSignature,
                    (functionProperties, arguments) -> new FunctionExpression(BuiltinFunctionName.NESTED.getName(), arguments) {
                        @Override
    ...
    public static class OpenSearchFunction extends FunctionExpression {
        ...
        public ExprValue valueOf(Environment<Expression, ExprValue> valueEnv) {
            throw new UnsupportedOperationException(
                String.format("OpenSearch defined function [%s] is only supported in WHERE and HAVING clause.", functionName)
            );
        }
