...
public class TextFunction {
    ...
    private DefaultFunctionResolver substringSubstr(FunctionName functionName) {
        return define(
            ...
            impl(nullMissingHandling(TextFunction::exprSubstrStartLength), STRING, STRING, INTEGER, INTEGER)
        );
    }
    ...
    private DefaultFunctionResolver ltrim() {
        return define(
            ...
            impl(nullMissingHandling((v) -> new ExprStringValue(v.stringValue().stripLeading())), STRING, STRING)
        );
    }
    ...
    private DefaultFunctionResolver rtrim() {
        return define(
            ...
            impl(nullMissingHandling((v) -> new ExprStringValue(v.stringValue().stripTrailing())), STRING, STRING)
        );
    }
    ...
    private DefaultFunctionResolver trim() {
        return define(
            ...
            impl(nullMissingHandling((v) -> new ExprStringValue(v.stringValue().trim())), STRING, STRING)
        );
    }
    ...
    private DefaultFunctionResolver lower() {
        return define(
            ...
            impl(nullMissingHandling((v) -> new ExprStringValue((v.stringValue().toLowerCase()))), STRING, STRING)
        );
    }
    ...
    private DefaultFunctionResolver upper() {
        return define(
            ...
            impl(nullMissingHandling((v) -> new ExprStringValue((v.stringValue().toUpperCase()))), STRING, STRING)
        );
    }
            ...
            funcName -> Pair.of(
                ...
                (funcProp, args) -> new FunctionExpression(funcName, args) {
                    ...
                    public ExprValue valueOf(Environment<Expression, ExprValue> valueEnv) {
                        List<ExprValue> exprValues = args.stream().map(arg -> arg.valueOf(valueEnv)).collect(Collectors.toList());
    ...
    private DefaultFunctionResolver concat_ws() {
        return define(
            ...
            impl(
                nullMissingHandling((sep, str1, str2) -> new ExprStringValue(str1.stringValue() + sep.stringValue() + str2.stringValue())),
                STRING,
                STRING,
                STRING,
                STRING
    ...
    private DefaultFunctionResolver length() {
        return define(
            ...
            impl(nullMissingHandling((str) -> new ExprIntegerValue(str.stringValue().getBytes().length)), INTEGER, STRING)
        );
    }
    ...
    private DefaultFunctionResolver strcmp() {
        return define(
            ...
            impl(
                nullMissingHandling(
                    (str1, str2) -> new ExprIntegerValue(Integer.compare(str1.stringValue().compareTo(str2.stringValue()), 0))
                ),
                INTEGER,
                STRING,
                STRING
    ...
    private DefaultFunctionResolver right() {
        return define(BuiltinFunctionName.RIGHT.getName(), impl(nullMissingHandling(TextFunction::exprRight), STRING, STRING, INTEGER));
    }
    ...
    private DefaultFunctionResolver left() {
        return define(BuiltinFunctionName.LEFT.getName(), impl(nullMissingHandling(TextFunction::exprLeft), STRING, STRING, INTEGER));
    }
    ...
    private DefaultFunctionResolver ascii() {
        return define(BuiltinFunctionName.ASCII.getName(), impl(nullMissingHandling(TextFunction::exprAscii), INTEGER, STRING));
    }
    ...
    private DefaultFunctionResolver locate() {
        return define(
            ...
            impl(
                nullMissingHandling((SerializableBiFunction<ExprValue, ExprValue, ExprValue>) TextFunction::exprLocate),
                INTEGER,
                STRING,
                STRING
            ),
            impl(
                nullMissingHandling((SerializableTriFunction<ExprValue, ExprValue, ExprValue, ExprValue>) TextFunction::exprLocate),
                INTEGER,
                STRING,
                STRING,
                INTEGER
    ...
    private DefaultFunctionResolver reverse() {
        return define(BuiltinFunctionName.REVERSE.getName(), impl(nullMissingHandling(TextFunction::exprReverse), STRING, STRING));
    }
