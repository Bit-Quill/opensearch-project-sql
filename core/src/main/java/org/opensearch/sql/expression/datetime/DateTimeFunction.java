...
public class DateTimeFunction {
    ...
    private static final Map<String, String> extract_formats = ImmutableMap.<String, String>builder()
    ...
    private static final Table<String, String, String> formats = ImmutableTable.<String, String, String>builder()
        .put("date", "usa", "%m.%d.%Y")
        .put("date", "jis", "%Y-%m-%d")
        .put("date", "iso", "%Y-%m-%d")
        .put("date", "eur", "%d.%m.%Y")
    ...
    private FunctionResolver now(FunctionName functionName) {
        return define(
            functionName,
            implWithProperties(functionProperties -> new ExprDatetimeValue(formatNow(functionProperties.getQueryStartClock())), DATETIME)
        );
    }
    ...
    private FunctionResolver sysdate() {
        return define(
            ...
            implWithProperties(functionProperties -> new ExprDatetimeValue(formatNow(Clock.systemDefaultZone())), DATETIME),
            FunctionDSL.implWithProperties(
                (functionProperties, v) -> new ExprDatetimeValue(formatNow(Clock.systemDefaultZone(), v.integerValue())),
                DATETIME,
                INTEGER
            )
        );
    ...
    private FunctionResolver curtime(FunctionName functionName) {
        return define(
            functionName,
            implWithProperties(
                functionProperties -> new ExprTimeValue(formatNow(functionProperties.getQueryStartClock()).toLocalTime()),
                TIME
            )
        );
    }
    ...
    private FunctionResolver curdate(FunctionName functionName) {
        return define(
            functionName,
            implWithProperties(
                functionProperties -> new ExprDateValue(formatNow(functionProperties.getQueryStartClock()).toLocalDate()),
                DATE
            )
        );
    }
    ...
    private Stream<SerializableFunction<?, ?>> get_date_add_date_sub_signatures(
        ...
        return Stream.of(
            ...
            implWithProperties(nullMissingHandlingWithProperties(function), DATETIME, DATETIME, INTERVAL),
            implWithProperties(nullMissingHandlingWithProperties(function), DATETIME, TIMESTAMP, INTERVAL),
    ...
    private DefaultFunctionResolver adddate() {
        return define(
            ...
            (SerializableFunction<FunctionName, Pair<FunctionSignature, FunctionBuilder>>[]) (Stream.concat(
    ...
    private DefaultFunctionResolver addtime() {
        return define(
            ...
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), TIME, TIME, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), TIME, TIME, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), TIME, TIME, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), TIME, TIME, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, DATETIME, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, DATETIME, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, DATETIME, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, DATETIME, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, DATE, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, DATE, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, DATE, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, DATE, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, TIMESTAMP, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, TIMESTAMP, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, TIMESTAMP, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprAddTime), DATETIME, TIMESTAMP, TIMESTAMP)
        );
    }
    ...
    private DefaultFunctionResolver convert_tz() {
        return define(
            ...
            impl(nullMissingHandling(DateTimeFunction::exprConvertTZ), DATETIME, DATETIME, STRING, STRING),
            impl(nullMissingHandling(DateTimeFunction::exprConvertTZ), DATETIME, STRING, STRING, STRING)
        );
    }
    ...
    private DefaultFunctionResolver datediff() {
        return define(
            ...
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, DATE, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, DATETIME, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, DATE, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, DATETIME, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, DATE, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, TIME, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, TIME, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, TIMESTAMP, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, DATE, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, TIMESTAMP, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, TIMESTAMP, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, TIME, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, TIMESTAMP, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, DATETIME, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, TIME, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprDateDiff), LONG, DATETIME, TIME)
        );
    }
    ...
    private DefaultFunctionResolver date_add() {
        return define(
            ...
            (SerializableFunction<FunctionName, Pair<FunctionSignature, FunctionBuilder>>[]) get_date_add_date_sub_signatures(
                DateTimeFunction::exprAddDateInterval
            ).toArray(SerializableFunction<?, ?>[]::new)
        );
    }
    ...
    private DefaultFunctionResolver date_sub() {
        return define(
            ...
            (SerializableFunction<FunctionName, Pair<FunctionSignature, FunctionBuilder>>[]) get_date_add_date_sub_signatures(
                DateTimeFunction::exprSubDateInterval
            ).toArray(SerializableFunction<?, ?>[]::new)
        );
    }
        ...
        return define(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, arg) -> DateTimeFunction.dayOfMonthToday(functionProperties.getQueryStartClock())
                ),
                INTEGER,
                TIME
            ),
    ...
    private DefaultFunctionResolver dayOfWeek(FunctionName name) {
        return define(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, arg) -> DateTimeFunction.dayOfWeekToday(functionProperties.getQueryStartClock())
                ),
                INTEGER,
                TIME
            ),
    ...
    private DefaultFunctionResolver dayOfYear(BuiltinFunctionName dayOfYear) {
        return define(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, arg) -> DateTimeFunction.dayOfYearToday(functionProperties.getQueryStartClock())
                ),
                INTEGER,
                TIME
            ),
    ...
    private DefaultFunctionResolver extract() {
        return define(
            ...
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprExtractForTime), LONG, STRING, TIME),
    ...
    private DefaultFunctionResolver from_days() {
        return define(BuiltinFunctionName.FROM_DAYS.getName(), impl(nullMissingHandling(DateTimeFunction::exprFromDays), DATE, LONG));
    }
    ...
    private DefaultFunctionResolver last_day() {
        return define(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, arg) -> DateTimeFunction.exprLastDayToday(functionProperties.getQueryStartClock())
                ),
                DATE,
                TIME
            ),
    ...
    private DefaultFunctionResolver month(BuiltinFunctionName month) {
        return define(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, arg) -> DateTimeFunction.monthOfYearToday(functionProperties.getQueryStartClock())
                ),
                INTEGER,
                TIME
            ),
    ...
    private DefaultFunctionResolver subtime() {
        return define(
            ...
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), TIME, TIME, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), TIME, TIME, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), TIME, TIME, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), TIME, TIME, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, DATETIME, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, DATETIME, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, DATETIME, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, DATETIME, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, DATE, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, DATE, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, DATE, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, DATE, TIMESTAMP),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, TIMESTAMP, TIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, TIMESTAMP, DATE),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, TIMESTAMP, DATETIME),
            implWithProperties(nullMissingHandlingWithProperties(DateTimeFunction::exprSubTime), DATETIME, TIMESTAMP, TIMESTAMP)
        );
    }
    ...
    private DefaultFunctionResolver str_to_date() {
        return define(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, arg, format) -> DateTimeFunction.exprStrToDate(functionProperties, arg, format)
                ),
                DATETIME,
                STRING,
                STRING
    ...
    private DefaultFunctionResolver timediff() {
        return define(BuiltinFunctionName.TIMEDIFF.getName(), impl(nullMissingHandling(DateTimeFunction::exprTimeDiff), TIME, TIME, TIME));
    }
    ...
    private DefaultFunctionResolver timestamp() {
        return define(
            ...
            impl(nullMissingHandling((v1, v2) -> exprAddTime(FunctionProperties.None, v1, v2)), TIMESTAMP, TIMESTAMP, TIMESTAMP)
        );
    }
    ...
    private DefaultFunctionResolver timestampdiff() {
        return define(
            ...
            impl(nullMissingHandling(DateTimeFunction::exprTimestampDiff), DATETIME, STRING, DATETIME, DATETIME),
            impl(nullMissingHandling(DateTimeFunction::exprTimestampDiff), DATETIME, STRING, DATETIME, TIMESTAMP),
            impl(nullMissingHandling(DateTimeFunction::exprTimestampDiff), DATETIME, STRING, TIMESTAMP, DATETIME),
            impl(nullMissingHandling(DateTimeFunction::exprTimestampDiff), DATETIME, STRING, TIMESTAMP, TIMESTAMP),
            implWithProperties(
                nullMissingHandlingWithProperties(
    ...
    private FunctionResolver unix_timestamp() {
        return define(
            ...
            implWithProperties(functionProperties -> DateTimeFunction.unixTimeStamp(functionProperties.getQueryStartClock()), LONG),
    ...
    private DefaultFunctionResolver week(BuiltinFunctionName week) {
        return define(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, arg) -> DateTimeFunction.weekOfYearToday(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, time, modeArg) -> DateTimeFunction.weekOfYearToday(
                        modeArg,
                        functionProperties.getQueryStartClock()
                    )
                ),
    ...
    private DefaultFunctionResolver yearweek() {
        return define(
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, arg) -> yearweekToday(DEFAULT_WEEK_OF_YEAR_MODE, functionProperties.getQueryStartClock())
                ),
                INTEGER,
                TIME
            ),
            ...
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, time, modeArg) -> yearweekToday(modeArg, functionProperties.getQueryStartClock())
                ),
                INTEGER,
                TIME,
                INTEGER
    ...
    private DefaultFunctionResolver date_format() {
        return define(
            ...
            impl(nullMissingHandling(DateTimeFormatterUtil::getFormattedDate), STRING, DATETIME, STRING),
            implWithProperties(
                nullMissingHandlingWithProperties(
                    (functionProperties, time, formatString) -> DateTimeFormatterUtil.getFormattedDateOfToday(
                        formatString,
                        time,
                        functionProperties.getQueryStartClock()
                    )
            ...
            impl(nullMissingHandling(DateTimeFormatterUtil::getFormattedDate), STRING, TIMESTAMP, STRING)
        );
    }
     ...
     */
    private ExprValue exprAddDateInterval(FunctionProperties functionProperties, ExprValue datetime, ExprValue interval) {
    ...
    private DefaultFunctionResolver time_format() {
        return define(
            ...
            impl(nullMissingHandling(DateTimeFormatterUtil::getFormattedTime), STRING, DATETIME, STRING),
            impl(nullMissingHandling(DateTimeFormatterUtil::getFormattedTime), STRING, TIME, STRING),
            impl(nullMissingHandling(DateTimeFormatterUtil::getFormattedTime), STRING, TIMESTAMP, STRING)
        );
    }
     ...
     */
    private ExprValue exprAddDateDays(FunctionProperties functionProperties, ExprValue datetime, ExprValue days) {
     ...
     * @return Datetime calculated.
     */
    private ExprValue exprDateApplyDays(FunctionProperties functionProperties, ExprValue datetime, Long days, Boolean isAdd) {
        if (datetime.type() == DATE) {
            return new ExprDateValue(isAdd ? datetime.dateValue().plusDays(days) : datetime.dateValue().minusDays(days));
        }
        var dt = extractDateTime(datetime, functionProperties);
     ...
     */
    private ExprValue exprApplyTime(FunctionProperties functionProperties, ExprValue temporal, ExprValue temporalDelta, Boolean isAdd) {
        ...
        return temporal.type() == TIME ? new ExprTimeValue(result.toLocalTime()) : new ExprDatetimeValue(result);
    }
     ...
     * @return A value calculated.
     */
    private ExprValue exprAddTime(FunctionProperties functionProperties, ExprValue temporal, ExprValue temporalDelta) {
    ...
    private ExprValue exprConvertTZ(ExprValue startingDateTime, ExprValue fromTz, ExprValue toTz) {
        ...
        try {
            ...
            // MySQL standard.
            if (!DateTimeUtils.isValidMySqlTimeZoneId(convertedFromTz) || !DateTimeUtils.isValidMySqlTimeZoneId(convertedToTz)) {
     ...
     * @return The diff.
     */
    private ExprValue exprDateDiff(FunctionProperties functionProperties, ExprValue first, ExprValue second) {
        // java inverses the value, so we have to swap 1 and 2
        return new ExprLongValue(DAYS.between(extractDate(second, functionProperties), extractDate(first, functionProperties)));
    }
    ...
    private ExprValue exprDateTime(ExprValue dateTime, ExprValue timeZone) {
        ...
        try {
            LocalDateTime ldtFormatted = LocalDateTime.parse(dateTime.stringValue(), DATE_TIME_FORMATTER_STRICT_WITH_TZ);
            if (timeZone.isNull()) {
        ...
        try {
            ZonedDateTime zdtWithZoneOffset = ZonedDateTime.parse(dateTime.stringValue(), DATE_TIME_FORMATTER_STRICT_WITH_TZ);
    ...
    private ExprValue exprExtractForTime(FunctionProperties functionProperties, ExprValue part, ExprValue time) {
        return formatExtractFunction(part, new ExprDatetimeValue(extractDateTime(time, functionProperties)));
    }
    ...
    private LocalDateTime exprFromUnixTimeImpl(ExprValue time) {
        return LocalDateTime.ofInstant(Instant.ofEpochSecond((long) Math.floor(time.doubleValue())), UTC_ZONE_ID)
    ...
    private ExprValue exprGetFormat(ExprValue type, ExprValue format) {
        if (formats.contains(type.stringValue().toLowerCase(), format.stringValue().toLowerCase())) {
            return new ExprStringValue(formats.get(type.stringValue().toLowerCase(), format.stringValue().toLowerCase()));
        }
    ...
    private ExprValue exprPeriodAdd(ExprValue period, ExprValue months) {
        ...
        return new ExprIntegerValue(Integer.parseInt(res.substring(0, res.length() - 2))); // Remove the day part, .eg. 20070101 -> 200701
    }
     ...
     */
    private ExprValue exprSubDateDays(FunctionProperties functionProperties, ExprValue date, ExprValue days) {
     ...
     */
    private ExprValue exprSubDateInterval(FunctionProperties functionProperties, ExprValue datetime, ExprValue expr) {
     ...
     * @return A value calculated.
     */
    private ExprValue exprSubTime(FunctionProperties functionProperties, ExprValue temporal, ExprValue temporalDelta) {
        return exprApplyTime(functionProperties, temporal, temporalDelta, false);
    }

    private ExprValue exprStrToDate(FunctionProperties fp, ExprValue dateTimeExpr, ExprValue formatStringExp) {
    ...
    private ExprValue exprTimestampDiff(ExprValue partExpr, ExprValue startTimeExpr, ExprValue endTimeExpr) {
        return getTimeDifference(partExpr.stringValue(), startTimeExpr.datetimeValue(), endTimeExpr.datetimeValue());
    }

    private ExprValue exprTimestampDiffForTimeType(
        FunctionProperties fp,
        ExprValue partExpr,
        ExprValue startTimeExpr,
        ExprValue endTimeExpr
    ) {
        return getTimeDifference(partExpr.stringValue(), extractDateTime(startTimeExpr, fp), extractDateTime(endTimeExpr, fp));
    }
    ...
    private ExprValue exprUtcTimeStamp(FunctionProperties functionProperties) {
        var zdt = ZonedDateTime.now(functionProperties.getQueryStartClock()).withZoneSameInstant(UTC_ZONE_ID);
    ...
    private ExprValue exprToSeconds(ExprValue date) {
        return new ExprLongValue(date.datetimeValue().toEpochSecond(ZoneOffset.UTC) + DAYS_0000_TO_1970 * SECONDS_PER_DAY);
    }
    ...
    private ExprValue exprToSecondsForIntType(ExprValue dateExpr) {
        try {
            ...
            LocalDate date = LocalDate.parse(String.valueOf(dateExpr.integerValue()), getFormatter(dateExpr.integerValue()));

            return new ExprLongValue(date.toEpochSecond(LocalTime.MIN, ZoneOffset.UTC) + DAYS_0000_TO_1970 * SECONDS_PER_DAY);
    ...
    private Double unixTimeStampOfImpl(ExprValue value) {
        ...
        switch ((ExprCoreType) value.type()) {
            ...
            case DATETIME:
                return value.datetimeValue().toEpochSecond(ZoneOffset.UTC) + value.datetimeValue().getNano() / 1E9;
            case TIMESTAMP:
    ...
    private LocalDateTime formatNow(Clock clock, Integer fsp) {
        ...
        var nano = new BigDecimal(res.getNano()).setScale(fsp - defaultPrecision, RoundingMode.DOWN).intValue();
