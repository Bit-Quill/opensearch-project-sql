...
public class BinaryPredicateOperator {
     ...
     */
    private static Table<ExprValue, ExprValue, ExprValue> andTable = new ImmutableTable.Builder<ExprValue, ExprValue, ExprValue>().put(
        LITERAL_TRUE,
        LITERAL_TRUE,
        LITERAL_TRUE
    )
     ...
     * MISSING MISSING MISSING
     */
    private static Table<ExprValue, ExprValue, ExprValue> orTable = new ImmutableTable.Builder<ExprValue, ExprValue, ExprValue>().put(
        LITERAL_TRUE,
        LITERAL_TRUE,
        LITERAL_TRUE
    )
     ...
     * MISSING MISSING MISSING
     */
    private static Table<ExprValue, ExprValue, ExprValue> xorTable = new ImmutableTable.Builder<ExprValue, ExprValue, ExprValue>().put(
        LITERAL_TRUE,
        LITERAL_TRUE,
        LITERAL_FALSE
    )
    ...
    private static DefaultFunctionResolver or() {
        return define(BuiltinFunctionName.OR.getName(), impl((v1, v2) -> lookupTableFunction(v1, v2, orTable), BOOLEAN, BOOLEAN, BOOLEAN));
    }
    ...
    private static DefaultFunctionResolver equal() {
        return define(
            ...
            ExprCoreType.coreTypes()
                .stream()
                .map(type -> impl(nullMissingHandling((v1, v2) -> ExprBooleanValue.of(v1.equals(v2))), BOOLEAN, type, type))
    ...
    private static DefaultFunctionResolver less() {
        return define(
            ...
            ExprCoreType.coreTypes()
                .stream()
                .map(type -> impl(nullMissingHandling((v1, v2) -> ExprBooleanValue.of(v1.compareTo(v2) < 0)), BOOLEAN, type, type))
                .collect(Collectors.toList())
        );
    }
    ...
    private static DefaultFunctionResolver like() {
        return define(BuiltinFunctionName.LIKE.getName(), impl(nullMissingHandling(OperatorUtils::matches), BOOLEAN, STRING, STRING));
    }
    ...
    private static DefaultFunctionResolver notLike() {
        return define(
            ...
            impl(nullMissingHandling((v1, v2) -> UnaryPredicateOperator.not(OperatorUtils.matches(v1, v2))), BOOLEAN, STRING, STRING)
        );
    }
