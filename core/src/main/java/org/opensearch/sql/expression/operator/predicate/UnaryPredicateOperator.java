...
public class UnaryPredicateOperator {
    ...
    private static DefaultFunctionResolver not() {
        return FunctionDSL.define(BuiltinFunctionName.NOT.getName(), FunctionDSL.impl(UnaryPredicateOperator::not, BOOLEAN, BOOLEAN));
    }
    ...
    private static DefaultFunctionResolver ifFunction() {
        ...
        List<SerializableFunction<FunctionName, org.apache.commons.lang3.tuple.Pair<FunctionSignature, FunctionBuilder>>> functionsOne =
            typeList.stream().map(v -> impl((UnaryPredicateOperator::exprIf), v, BOOLEAN, v, v)).collect(Collectors.toList());
    ...
    private static DefaultFunctionResolver ifNull() {
        ...
        List<SerializableFunction<FunctionName, org.apache.commons.lang3.tuple.Pair<FunctionSignature, FunctionBuilder>>> functionsOne =
            typeList.stream().map(v -> impl((UnaryPredicateOperator::exprIfNull), v, v, v)).collect(Collectors.toList());
    ...
    private static DefaultFunctionResolver nullIf() {
        ...
        DefaultFunctionResolver functionResolver = FunctionDSL.define(
            functionName,
            typeList.stream().map(v -> impl((UnaryPredicateOperator::exprNullIf), v, v, v)).collect(Collectors.toList())
        );
        return functionResolver;
    }
