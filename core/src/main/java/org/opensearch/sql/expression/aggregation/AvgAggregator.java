...
public class AvgAggregator extends Aggregator<AvgAggregator.AvgState> {
    ...
    public AvgState create() {
        switch (dataType) {
            ...
            default: // unreachable code - we don't expose signatures for unsupported types
                throw new IllegalArgumentException(String.format("avg aggregation over %s type is not supported", dataType));
        }
    }
    ...
    protected static class DateAvgState extends AvgState {
        ...
        public ExprValue result() {
            ...
            return new ExprDateValue(
                new ExprTimestampValue(Instant.ofEpochMilli(DSL.divide(DSL.literal(total), DSL.literal(count)).valueOf().longValue()))
                    .dateValue()
            );
        }
        ...
        @Override
        protected AvgState iterate(ExprValue value) {
            total = DSL.add(DSL.literal(total), DSL.literal(value.timestampValue().toEpochMilli())).valueOf();
            return super.iterate(value);
        }
    }
    ...
    protected static class TimestampAvgState extends AvgState {
        ...
        public ExprValue result() {
            ...
            return new ExprTimestampValue(Instant.ofEpochMilli(DSL.divide(DSL.literal(total), DSL.literal(count)).valueOf().longValue()));
        }
    ...
    protected static class TimeAvgState extends AvgState {
        ...
        public ExprValue result() {
            ...
            return new ExprTimeValue(LocalTime.MIN.plus(DSL.divide(DSL.literal(total), DSL.literal(count)).valueOf().longValue(), MILLIS));
        }

        @Override
        protected AvgState iterate(ExprValue value) {
            total = DSL.add(DSL.literal(total), DSL.literal(MILLIS.between(LocalTime.MIN, value.timeValue()))).valueOf();
            return super.iterate(value);
        }
    }
}
