...
public class FunctionSignature {
    ...
    public int match(FunctionSignature functionSignature) {
        ...
        if (!functionName.equals(functionSignature.getFunctionName()) || paramTypeList.size() != functionTypeList.size()) {
            return NOT_MATCH;
        }
    ...
    public String formatTypes() {
        return getParamTypeList().stream().map(ExprType::typeName).collect(Collectors.joining(",", "[", "]"));
    }
