    ...
    public Pair<FunctionSignature, FunctionBuilder> resolve(FunctionSignature unresolvedSignature) {
        ...
        for (int i = 1; i < paramTypes.size(); i++) {
            ...
            if (!ExprCoreType.STRING.equals(paramType)) {
                throw new SemanticCheckException(getWrongParameterErrorMessage(i, paramType, ExprCoreType.STRING));
            }
        }

        FunctionBuilder buildFunction = (functionProperties, args) -> new OpenSearchFunctions.OpenSearchFunction(functionName, args);
