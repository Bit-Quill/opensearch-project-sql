...
public class BuiltinFunctionRepository {
    ...
    public FunctionImplementation compile(
        ...
        FunctionBuilder resolvedFunctionBuilder = resolve(
            ...
            new FunctionSignature(functionName, expressions.stream().map(Expression::type).collect(Collectors.toList()))
        );
    ...
    public FunctionBuilder resolve(Collection<FunctionResolver> dataSourceFunctionResolver, FunctionSignature functionSignature) {
        ...
        return resolve(functionSignature, dataSourceFunctionMap).or(() -> resolve(functionSignature, functionResolverMap))
            .orElseThrow(
                () -> new ExpressionEvaluationException(String.format("unsupported function name: %s", functionSignature.getFunctionName()))
            );
    }
    ...
    private Optional<FunctionBuilder> resolve(
        ...
        if (functionResolverMap.containsKey(functionName)) {
            ...
            if (isCastFunction(functionName) || FunctionSignature.isVarArgFunction(targetTypes) || sourceTypes.equals(targetTypes)) {
    ...
    private Function<FunctionProperties, Expression> cast(Expression arg, ExprType targetType) {
        ...
        if (castFunctionName == null) {
            throw new ExpressionEvaluationException(StringUtils.format("Type conversion to type %s is not supported", targetType));
        }
        return functionProperties -> (Expression) compile(functionProperties, castFunctionName, List.of(arg));
    }
}
